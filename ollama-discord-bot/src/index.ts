...
const sessions = new Map<Snowflake, LongChatSession>();
...
const WORKSPACE_ADMINS = new Set(
  (process.env.WORKSPACE_ADMINS || "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean)
);
...
const workspaceSessions = new Map<Snowflake, WorkspaceSession>();

const isWorkspaceAdmin = (userId: Snowflake) => WORKSPACE_ADMINS.has(userId);

const resolveWorkspaceFile = (dir: string, rel: string) => {
  const target = path.resolve(dir, rel);
  if (target === dir || target.startsWith(`${dir}${path.sep}`)) return target;
  throw new Error("Path must stay within workspace directory");
};

const runDocker = async (cmd: string, options?: { cwd?: string }) => {
  ...
  if (!res.ok && /sudo: a password is required/i.test(res.stderr || res.stdout)) {
    return {
      ...
      stdout: "",
      stderr:
        "Docker needs passwordless sudo. Set USE_SUDO_DOCKER=0 if your user can run docker without sudo, or add NOPASSWD for docker in sudoers.",
    } as const;
  }
  return res;
};

const runDockerExec = async (containerId: string, command: string, workdir = "/workspace") => {
  return runDocker(`exec -w ${workdir} ${containerId} bash -lc ${JSON.stringify(command)}`);
};

const findUserSession = (userId: Snowflake) =>
  [...sessions.values()].find((s) => s.userId === userId);

const isGuildTextChannel = (channel: any): channel is TextChannel =>
...
const buildCodeblock = (body: string, lang: string) => `\u0060\u0060\u0060${lang}\n${body}\n\u0060\u0060\u0060`;

const truncateForDiscord = (text: string, limit = 1900) =>
...
const resolveWorkspacePath = (input: string) => {
  const target = path.resolve(workspaceRoot, input);
  if (target === workspaceRoot || target.startsWith(`${workspaceRoot}${path.sep}`)) return target;
  throw new Error("Path must stay within workspace root");
};

const readFileForReply = (inputPath: string) => {
  ...
  if (stat.size > MAX_FILE_BYTES) throw new Error("File too large (over 2MB)");
...
const splitCodeAndStdin = (input: string) => {
  const marker = "--stdin";
  const idx = input.indexOf(marker);
  if (idx === -1) return { code: input.trim(), stdin: "" };
  return {
...
const readAttachmentText = async (attachment: { url: string; name: string | null }) => {
  const res = await fetch(attachment.url);
  if (!res.ok) throw new Error(`Could not download attachment (${res.status})`);
  const len = Number(res.headers.get("content-length") || "0");
  if (len && len > MAX_ATTACHMENT_BYTES) throw new Error("Attachment too large (over 2MB)");
  const buf = Buffer.from(await res.arrayBuffer());
  if (buf.length > MAX_ATTACHMENT_BYTES) throw new Error("Attachment too large (over 2MB)");
  return buf.toString("utf-8");
};

const buildLongOutputResponse = (text: string, filename: string, intro?: string) => {
  const lines = text.split(/\r?\n/);
...
const workspaceBlockedCommand = (command: string) => {
  const patterns = [
    /\bsudo\b/i,
    /\bapt(-get)?\b/i,
    /\bdnf\b/i,
...
const findWorkspaceByUser = (userId: Snowflake) =>
  [...workspaceSessions.values()].find((s) => s.userId === userId);

const createWorkspace = async (guild: any, user: any) => {
  if (workspaceSessions.size >= WORKSPACE_MAX_CONCURRENT)
    throw new Error("Workspace limit reached. Try again later.");

  const existing = findWorkspaceByUser(user.id as Snowflake);
  if (existing) throw new Error(`You already have a workspace at <#${existing.channelId}>.`);
  ...
  if (!isGuildTextChannel(channel)) throw new Error("Could not create workspace channel.");
  ...
  const runResult = await runDocker(
    `run -d --name ${name} --network none --cpus=${WORKSPACE_CPU} --memory=${WORKSPACE_MEM} --pids-limit=${WORKSPACE_PIDS} -w /workspace -v ${dir}:/workspace:rw ${WORKSPACE_IMAGE} tail -f /dev/null`
  );
  ...
  workspaceSessions.set(channel.id as Snowflake, session);
  scheduleWorkspaceExpiry(session);
  return { channel, session };
};

const cleanupWorkspace = async (session: WorkspaceSession, reason = "workspace closed") => {
  ...
  try {
    ...
    if (channel && "delete" in channel) {
      await (channel as any).delete(reason).catch(() => {});
    }
  } catch (err) {
...
const scheduleWorkspaceExpiry = (session: WorkspaceSession) => {
  const delay = Math.max(1000, session.expiresAt - Date.now());
  session.timer = setTimeout(async () => {
    const channel = client.channels.cache.get(session.channelId as Snowflake);
    if (channel && (channel as any).isTextBased?.()) {
      (channel as any).send?.("Workspace expired. Cleaning up.").catch(() => {});
    }
    await cleanupWorkspace(session, "workspace expired");
  }, delay);
};

const saveWorkspaceAttachment = async (
...
const handleWorkspaceMessage = async (session: WorkspaceSession, message: Message) => {
  const isOwner = message.author.id === session.userId;
  const isAdmin = isWorkspaceAdmin(message.author.id as Snowflake);
  if (!isOwner && !isAdmin) {
  ...
  const inWorkspaceDir = (p: string) => resolveWorkspaceFile(session.dir, p);

  const runWorkspaceCode = async (lang: "py" | "cpp" | "js", body: string) => {
    ...
    if (fileExists) {
      ...
      if (lang === "py") {
        ...
        const compile = await runDockerExec(
          session.containerId,
          `g++ -std=c++17 -O0 -pipe -o ${bin} ${containerPath}`
        );
        if (!compile.ok) {
    ...
    if (lang === "py") {
      ...
      const compile = await runDockerExec(
        session.containerId,
        `g++ -std=c++17 -O0 -pipe -o ${bin} ${relTemp}`
      );
      if (!compile.ok) {
        await message.reply({ content: formatOutputs("", compile.stderr || compile.stdout, "") });
        return;
...
const promptForStdin = async (channel: any, userId: string, label: string) => {
...
const ensureBinary = async (binary: string) => {
  try {
...
const runCommand = async (command: string, cwd?: string, stdinText?: string, env?: NodeJS.ProcessEnv) => {
  ...
  if (destructive) {
    return { ok: false, stdout: "", stderr: "Blocked destructive command" } as const;
  }
  ...
  try {
    ...
    if (tmpIn) fs.rmSync(tmpIn, { recursive: true, force: true });
    return { ok: true, stdout: stdout || "", stderr: stderr || "" } as const;
  } catch (err: any) {
    if (tmpIn) fs.rmSync(tmpIn, { recursive: true, force: true });
    return {
      ok: false,
...
const makeBar = (pct: number, width = 14) => {
...
const readDisk = async (target: string) => {
  const res = await runCommand(`df -k ${JSON.stringify(target)}`);
  if (!res.ok) throw new Error(res.stderr || "df failed");
...
const buildStatusEmbed = (stats: Awaited<ReturnType<typeof collectHostStats>>) => {
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      `Active workspaces: ${workspaceSessions.size}/${WORKSPACE_MAX_CONCURRENT} | Image: ${WORKSPACE_IMAGE}`
    )
    .addFields(
      {
        name: "CPU",
...
const formatOutputs = (stdout: string, stderr: string, lang = "") => {
  const parts: string[] = [];
...
const runCpp = async (code: string, stdinText?: string) => {
...
const runCppFile = async (filePath: string, cwd: string, stdinText?: string) => {
  await ensureBinary("g++");
...
const runPython = async (code: string, stdinText?: string) => {
...
const runPythonFile = async (filePath: string, cwd: string, stdinText?: string) => {
  await ensureBinary("python3");
...
const runJs = async (code: string, stdinText?: string) => {
...
const runJsFile = async (filePath: string, cwd: string, stdinText?: string) => {
  await ensureBinary("node");
...
const wikiLookup = async (query: string) => {
...
const isAuthorized = (req: http.IncomingMessage, urlObj: URL) => {
...
const convertAdrOfToLegacyAdr = (code: string): { converted: string; changed: boolean } => {
  const converted = code.replace(/adr_of\s+([A-Za-z_][\w]*)/gi, "adr($1)");
  return { converted, changed: converted !== code };
};

const convertLabelColonToLegacyLbl = (code: string): { converted: string; changed: boolean } => {
...
const shouldSkipFx = (code: string): boolean => {
...
const startAdminServer = () => {
  const server = http.createServer((req: http.IncomingMessage, res: http.ServerResponse) => {
    ...
    if (req.method === "GET" && urlObj.pathname === "/api/transcript") {
      ...
      const withinRoot =
        target === transcriptsRoot || target.startsWith(`${transcriptsRoot}${path.sep}`);
      if (!withinRoot) {
        res.writeHead(400, { "content-type": "text/plain" });
...
const formatTranscript = (log: ChatTurn[]) =>
  log.map((entry) => `[${entry.role}] ${entry.content}`).join("\n");

const saveTranscript = (session: LongChatSession) => {
  try {
...
const resetInactivityTimer = (session: LongChatSession, channel: TextChannel) => {
  if (session.timer) clearTimeout(session.timer);
  session.timer = setTimeout(async () => {
...
const endSession = async (session: LongChatSession, reason = "Chat finished") => {
  if (session.timer) clearTimeout(session.timer);
  sessions.delete(session.channelId);
  saveTranscript(session);
  try {
    const channel = await client.channels.fetch(session.channelId);
    if (!channel || !isGuildTextChannel(channel)) return;
...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption((opt: SlashCommandStringOption) =>
      opt.setName("code").setDescription("Program body").setRequired(true)
    ),
  new SlashCommandBuilder()
    .setName("fxcompile")
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption((opt: SlashCommandStringOption) =>
      opt.setName("code").setDescription("C++ source").setRequired(false)
    )
    .addAttachmentOption((opt) =>
      opt.setName("file").setDescription("Upload C++ source").setRequired(false)
    )
    .addStringOption((opt: SlashCommandStringOption) =>
      opt.setName("stdin").setDescription("Optional stdin").setRequired(false)
    ),
  new SlashCommandBuilder()
    .setName("pycomp")
    .setDescription("Run Python code")
    .addStringOption((opt: SlashCommandStringOption) =>
      opt.setName("code").setDescription("Python source").setRequired(false)
    )
    .addAttachmentOption((opt) =>
      opt.setName("file").setDescription("Upload Python source").setRequired(false)
    )
    .addStringOption((opt: SlashCommandStringOption) =>
      opt.setName("stdin").setDescription("Optional stdin").setRequired(false)
    ),
  new SlashCommandBuilder()
    ...
    .addStringOption((opt: SlashCommandStringOption) =>
      opt.setName("code").setDescription("JavaScript source").setRequired(false)
    )
    .addAttachmentOption((opt) =>
      opt.setName("file").setDescription("Upload JavaScript source").setRequired(false)
    )
    .addStringOption((opt: SlashCommandStringOption) =>
      opt.setName("stdin").setDescription("Optional stdin").setRequired(false)
    ),
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption((opt: SlashCommandStringOption) => {
      ...
      if (fxToolChoiceTuples.length) opt.addChoices(...fxToolChoiceTuples);
      return opt;
    })
    .addStringOption((opt: SlashCommandStringOption) =>
      opt
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption((opt: SlashCommandStringOption) =>
      opt.setName("hex").setDescription("Hex bytes to decompile").setRequired(true)
    ),
].map((c) => c.toJSON());
...
client.on("interactionCreate", async (interaction: Interaction) => {
  ...
  if (interaction.commandName === "stopchat") {
    const sessionByChannel = sessions.get(interaction.channelId as Snowflake);
    const sessionByUser = findUserSession(interaction.user.id as Snowflake);
  ...
  if (interaction.commandName === "longchat") {
    ...
    const existing = findUserSession(interaction.user.id as Snowflake);
    if (existing) {
      await interaction.reply({
    ...
    try {
      ...
      const transcriptPath = path.join(
        transcriptsDir,
        `${interaction.user.id}-${Date.now()}-${safeName}.txt`
      );

      const newSession: LongChatSession = {
      ...
      resetInactivityTimer(newSession, channel as TextChannel);

      await channel.send(
        `Long chat opened for <@${interaction.user.id}>. Topic: ${topic}. Use ${PREFIX_STOPCHAT} or /stopchat to finish and save the transcript.`
      );
      await interaction.reply({
  ...
  if (interaction.commandName === "fxcompile") {
    const requestedModel = interaction.options.getString("model") as FxModel | null;
    const code = interaction.options.getString("code", true);
    const format = interaction.options.getString("format") as "hex" | "key" | null;
  ...
  if (interaction.commandName === "fxtool") {
    ...
    if (!availableTools.includes(tool)) {
      await interaction.editReply(
        `Tool ${tool} not found. Available: ${availableTools.join(", ") || "none"}.`
      );
      return;
    }
  ...
  if (interaction.commandName === "compile") {
    ...
    if (!compilerAvailable()) {
      await interaction.editReply(
        "compiler assets missing. Add rom.bin, disas.txt, gadgets.txt, labels.txt, labels_sfr.txt, extensions.txt under compiler/."
      );
      return;
    }
    ...
    try {
      ...
      const response = buildLongOutputResponse(
        hex,
        "compile.txt",
        changed ? "[legacy shim]" : undefined
      );
      await interaction.editReply(response);
    } catch (err: any) {
  ...
  if (interaction.commandName === "decompile") {
    ...
    if (!decompilerAvailable()) {
      await interaction.editReply(
        "Decompiler missing. Ensure compiler/decompiler.py and assets exist."
      );
      return;
    }
  ...
  client.on("messageCreate", async (message: Message) => {
    ...
    const workspaceSession = workspaceSessions.get(message.channel.id as Snowflake);
    if (workspaceSession) {
    ...
    if (
      contentLower.startsWith(PREFIX_CHAT) ||
    ...
    if (content.startsWith(PREFIX_FXCOMP)) {
      ...
      const available = availableFxModels();
      if (!available.includes(modelToken as FxModel)) {
        await message.reply(
          `Unknown or unavailable model "${modelToken}". Available: ${available.join(", ") || "none"}.`
        );
        return;
      }
      ...
      try {
        const out = await compileFx(modelToken as FxModel, code, { format: "hex" });
    ...
    if (content.startsWith(PREFIX_FXTOOL)) {
      ...
      if (!body) {
        await message.reply(
          `Usage: ${PREFIX_FXTOOL} <tool.py> [args...] (available: ${availableFxTools().join(", ") || "none"})`
        );
        return;
      }
