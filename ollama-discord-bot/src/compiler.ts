...
export async function runCompile(input: string): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on("close", (code: number | null) => {
      if (code === 0) {
        ...
        if (warning && out) resolve(`${out}\n\n[warning]\n${warning}`);
        else if (warning) resolve(`[warning]\n${warning}`);
        else resolve(out);
      } else {
...
export function decompilerAvailable(): boolean {
  return fs.existsSync(DECOMPILER_ENTRY);
}

export async function runDecompile(input: string): Promise<string> {
